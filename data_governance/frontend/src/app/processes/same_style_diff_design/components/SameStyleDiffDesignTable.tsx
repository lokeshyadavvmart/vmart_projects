// src/app/processes/same_style_diff_design/components/SameStyleDiffDesignTable.tsx
...
import { fetchSameStyleDiffDesign } from '../api/fetchSameStyleDiffDesign';
import { Filters, DateRange, SameStyleDiffDesignRow } from '../types/sameStyleDiffDesignTypes';
...
export interface SameStyleDiffDesignTableHandle {
    exportCurrentView: () => void;
}

interface SameStyleDiffDesignTableProps {
    filters: Filters;
    dateRange: DateRange;
...
interface BlockCache {
    rows: SameStyleDiffDesignRow[];
    nextCursor: string | null;
}

export const SameStyleDiffDesignTable = forwardRef<SameStyleDiffDesignTableHandle, SameStyleDiffDesignTableProps>(({
    filters,
    dateRange,
    onDataLoaded,
}, ref) => {
        ...
        exportCurrentView: () => {
            if (gridRef.current?.api) {
                gridRef.current.api.exportDataAsCsv({
                    fileName: `same_style_diff_design_current_${new Date().toISOString().slice(0, 10)}.csv`,
                });
            }
        },
    }));
    ...
    const getRows = useCallback(
        async (params: IGetRowsParams) => {
            ...
            try {
                const response: FetchResult = await fetchSameStyleDiffDesign(
                    {
            ...
            <div className="ag-theme-quartz beautiful-grid w-full h-[600px]">
                <AgGridReact
                    ...
                    overlayNoRowsTemplate={
                        '<span class="ag-overlay-no-rows-center">No same style diff design records found</span>'
                    }
...
SameStyleDiffDesignTable.displayName = 'SameStyleDiffDesignTable';