...
DISTINCT_KEYS = ["site_code", "division", "section", "department", "vendor", "fabric"]
...
def fetch_all_distinct_values() -> Dict[str, Set[str]]:
    ...
    for col in DISTINCT_KEYS:
        query = f"SELECT DISTINCT {col} FROM governance.color_variants"
...
# -----------------------------
def get_color_variants(params, previous_distincts: Optional[Dict[str, Set[str]]] = None) -> Dict[str, Any]:
    conditions = ["1 = 1"]
    ...
    rows, columns = fetch_color_variants(where_clause, limit, return_columns=True)
...
# Full export
# -----------------------------
def get_full_color_variants(filters: Optional[Dict[str, Any]] = None):
    conditions = ["1 = 1"]
    if filters:
        for key, value in filters.items():
            if key not in ALLOWED_FILTERS:
