from app.repositories.duplicate_item_repo import fetch_duplicate_items
...
DISTINCT_KEYS = ["site_code", "division", "section", "department", "vendor"]
...
# ------------------------------
# NEW: fetch all distinct values at first request
# ------------------------------
def fetch_all_distinct_values() -> Dict[str, Set[str]]:
    distincts: Dict[str, Set[str]] = {}
    for col in DISTINCT_KEYS:
        query = f"SELECT DISTINCT {col} FROM governance.duplicate_items"
...
# Main function
# ------------------------------
def get_duplicate_items(params, previous_distincts: Optional[Dict[str, Set[str]]] = None) -> Dict[str, Any]:
    conditions = ["1 = 1"]
    ...
    rows, columns = fetch_duplicate_items(where_clause, limit, return_columns=True)
    ...
    if rows:
        last_row = rows[-1]
        try:
            next_cursor = str(last_row[5])  # order_date index
        except Exception:
            next_cursor = None
    ...
    # Convert sets to sorted lists for frontend
...
# ------------------------------
def get_full_duplicate_items(filters: Optional[Dict[str, Any]] = None):
    conditions = ["1 = 1"]
    if filters:
        for key, value in filters.items():
            if key not in ALLOWED_FILTERS:
